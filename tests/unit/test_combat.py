...
class TestCombatResolver:
    ...
    def test_damage_calculation_hit(self, reset_rng):
        ...
        for _ in range(10):
            ...
            damage, hit = resolver._calculate_damage(attacker, defender)
...
class TestAmmoConsumption:
    ...
    def test_ammo_consumed_on_attack(self, reset_rng):
        """攻撃時に弾薬消費"""
        attacker = Ship(1, "destroyer", "Attacker")
        defender = Ship(2, "destroyer", "Defender")
        
        initial_ammo = attacker.ammo
        resolver = CombatResolver()
        ...
        if hit:
            # 命中した場合、弾薬消費
            assert attacker.ammo == initial_ammo - attacker.ammo_per_shot
        else:
            # 外れた場合も弾薬消費（現在の仕様）
            assert attacker.ammo == initial_ammo - attacker.ammo_per_shot
